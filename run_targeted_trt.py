...
from monitor_gpt2 import TargetedDEGFMonitor
import numpy as np

def run_targeted_trt():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = HookedTransformer.from_pretrained("gpt2-small", device=device)

    with open("q2_targets.txt", "r") as f:
        target_heads = [tuple(map(int, line.strip().split(","))) for line in f if line.strip()]

    monitor = TargetedDEGFMonitor(model, target_heads)
    ...
    print(f"Monitoring {len(target_heads)} Q2 Target Heads")
    ...
    for task in tasks:
        ...
        # Targeted Q2 heads should show much stronger signals
